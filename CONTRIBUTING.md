...
- **本机/内网真实拓扑** — 真实主机名、内网 IP 段(`192.168.1.0/24` 等带具体部署信息的)、服务到内网 IP 的映射关系。注意区分:**通用代理端口**(`7890`、`7897` 等 Clash/代理软件默认端口)和**软件默认端口**(`9090`、`25565` 等)本身是公开常识,出现在代码/文档里不构成泄露,无需隐藏;只有"某端口绑定到某台真实内网机器"这种**映射关系**才需要避免入库
