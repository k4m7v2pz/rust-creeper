...
impl UniversalProtocol for ProtocolWrapper {
    ...
    fn connect(&self, host: &str, port: u16, _proxy: Option<&ProxyInfo>, listener: Arc<dyn SessionListener>) -> Box<dyn BotSession> {
        log::info!("[{}] Connecting {}:{} (1.21.1)", self.profile.name, host, port);
        Box::new(V1_21_1Session { username: self.profile.name.clone(), connected: true, listener })
    }
}

#[allow(dead_code)]
struct V1_21_1Session { username: String, connected: bool, listener: Arc<dyn SessionListener> }

impl BotSession for V1_21_1Session {
    fn is_connected(&self) -> bool { self.connected }
    fn send_chat(&mut self, msg: &str) { log::info!("[{}] Chat 1.21.1: {}", self.username, msg); }
    fn disconnect(&mut self, reason: &str) { self.connected = false; self.listener.on_disconnected(reason); }
}

impl Drop for V1_21_1Session { fn drop(&mut self) { if self.connected { self.disconnect("drop"); } } }
